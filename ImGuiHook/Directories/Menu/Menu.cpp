...
void Menu::Render(bool* p_open)
{
	...
	if (!ImGui::Begin("Anno 1800 Mod Menu v1.6", p_open, window_flags))
	{
		...
		ImGui::SliderFloat("UI Scale", &uiScale, 0.5f, 3.0f, "%.1fx");
		...
		if (ImGui::CollapsingHeader("CHEATS", 32))
		{
			...
			{
				if (ImGui::Checkbox("##1", &bES_active0)) { sendCommand("ts.Cheat.GlobalCheats.ToggleIgnoreFertilities()"); }
				ImGui::SameLine(90);
				ImGui::Text("Toggle ignore fertilities");

				if (ImGui::Checkbox("##2", &bES_active1)) { sendCommand("ts.Cheat.GlobalCheats.ToggleSuperShipSpeed()"); }
				ImGui::SameLine(90);
				ImGui::Text("Toggle super ship speed");

				if (ImGui::Checkbox("##3", &bES_active2)) { sendCommand("ts.Cheat.GlobalCheats.ToggleElectricity()"); }
				ImGui::SameLine(90);
				ImGui::Text("Toggle electricity");

				//if (ImGui::Checkbox("##4", &bES_active3)) { sendCommand("ts.Cheat.GlobalCheats.ToggleIgnoreBuildingCosts()"); }
				//ImGui::SameLine(90);
				//ImGui::Text("Toggle ignore building costs");

				//if (ImGui::Checkbox("##5", &bES_active4)) { sendCommand("ts.Cheat.GlobalCheats.ToggleProductivity()"); }
				//ImGui::SameLine(90);
				//ImGui::Text("Toggle productivity");

				if (ImGui::Button("Set##6")) { sendCommand("ts.Participants.CheatTotalTrade()"); }
				ImGui::SameLine(90);
				ImGui::Text("Total trade");

				if (ImGui::Button("Set##7")) { sendCommand("ts.Participants.CheatTotalWar()"); }
				ImGui::SameLine(90);
			...
			if (ImGui::TreeNode("Items"))
			{
				...
				ImGui::SameLine(90);
				ImGui::PushItemWidth(125);
				if (ImGui::BeginCombo(" (GUID) to current selected ship##inputi2", items[i2])) {
					// Scroll to selected item on popup open or after keyboard navigation.
					// Proportional scroll based on GetScrollMaxY() avoids coordinate-system
					// confusion (GetCursorPosY is scroll-relative inside a popup window).
					// SetScrollHereY then fine-tunes to exact center once the item is in range.
					if (ImGui::IsWindowAppearing()) i2_scroll = true;
					if (i2_scroll) {
						float max_scroll = ImGui::GetScrollMaxY();
						ImGui::SetScrollY(max_scroll > 0.0f
							? ((float)i2 / IM_ARRAYSIZE(items)) * max_scroll
							: i2 * ImGui::GetTextLineHeightWithSpacing());
					}
					ImGuiListClipper clipper;
					clipper.Begin(IM_ARRAYSIZE(items));
					while (clipper.Step()) {
						for (int j = clipper.DisplayStart; j < clipper.DisplayEnd; j++) {
							bool selected = (j == i2);
							if (ImGui::Selectable(items[j], selected))
								i2 = j;
							if (selected && i2_scroll) {
				...
				// First-letter keyboard navigation: press a letter to jump to the first item
				// starting with that letter; press the same letter again to cycle to the next.
				// WantTextInput is true only when an InputInt/InputText field is being edited.
				if (!ImGui::GetIO().WantTextInput) {
					ImGuiIO& io = ImGui::GetIO();
					for (int k = 0; k < io.InputQueueCharacters.Size; k++) {
						ImWchar wc = io.InputQueueCharacters[k];
						if (wc < 32 || wc > 126) continue;
						char lower = (char)tolower((unsigned char)wc);
						if (lower < 'a' || lower > 'z') continue;
						int i2_before = i2;
						// If already on a match, start from the next index to cycle
						int start = (items[i2][0] && tolower((unsigned char)items[i2][0]) == lower)
							? i2 + 1 : 0;
						bool found = false;
						for (int j = start; j < IM_ARRAYSIZE(items); j++) {
							if (items[j][0] && tolower((unsigned char)items[j][0]) == lower) {
								i2 = j; found = true; break;
							}
						}
						if (!found && start > 0) {
							// Wrap: no more matches after current, restart from beginning
						...
						if (i2 != i2_before) i2_scroll = true;
					}
				}
			...
			{
				if (ImGui::Button("Set##9")) { sendCommand("TextSources.TextSourceRoots.Discovery.ShowAll()"); }
				ImGui::SameLine(90);
				ImGui::Text("Discover whole map");

				if (ImGui::Button("Set##10")) { sendCommand("TextSources.TextSourceRoots.Discovery.HideAll()"); }
				ImGui::SameLine(90);
			...
			{
				if (ImGui::Button("Change##11")) { sendCommand("ts.Weather.SetChangeWind()"); }
				ImGui::SameLine(90);
			...
			//	ImGui::TreePop();
			//}
			if (ImGui::TreeNode("Infection"))
			{
				if (ImGui::Button("Burn##15")) { sendCommand("ts.Selection.Object.ShipIncident.CheatInfect(0)"); }
				ImGui::SameLine(90);
				ImGui::Text("Burn selected ship");

				if (ImGui::Button("Riot##16")) { sendCommand("ts.Selection.Object.ShipIncident.CheatInfect(1)"); }
				ImGui::SameLine(90);
				ImGui::Text("Riot selected ship");

				if (ImGui::Button("Disease##17")) { sendCommand("ts.Selection.Object.ShipIncident.CheatInfect(2)"); }
				ImGui::SameLine(90);
				ImGui::Text("Disease selected ship");

				if (ImGui::Button("Freeze##18")) { sendCommand("ts.Selection.Object.ShipIncident.CheatInfect(4)"); }
				ImGui::SameLine(90);
				ImGui::Text("Freeze selected ship");

				if (ImGui::Button("Random##19")) { sendCommand("ts.Selection.Object.ShipIncident.CheatInfectRandom()"); }
				ImGui::SameLine(90);
				ImGui::Text("Random infect selected ship");

				if (ImGui::Button("Uninfect##20")) { sendCommand("ts.Selection.Object.ShipIncident.CheatEndAnyInfection()"); }
				ImGui::SameLine(90);
				ImGui::Text("Uninfect selected ship");

				ImGui::TreePop();
			}
			//if (ImGui::TreeNode("UI"))
			//{
			//	if (ImGui::Button("Set##21")) { sendCommand("ts.Interface.SetUiScreenScaling(0.7)"); }
			//	ImGui::SameLine(90);
			//	ImGui::Text("Set UI screen scaling to 70%");
			//
			//	if (ImGui::Button("Set##22")) { sendCommand("ts.Interface.SetUiScreenScaling(1.0)"); }
			//	ImGui::SameLine(90);
			//	ImGui::Text("Set UI screen scaling to 100%");
			//
			//	if (ImGui::Button("Set##23")) { sendCommand("ts.Interface.SetUiScreenScaling(1.2)"); }
			//	ImGui::SameLine(90);
			//	ImGui::Text("Set UI screen scaling to 120%");
			//
			//	ImGui::TreePop();
			//}
